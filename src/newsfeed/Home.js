...
export default class Home extends Component {
    ...
    postMessage = (text) => fetch("https://cue-medical-management-app.herokuapp.com/posts", {
        method: "POST",
        headers: {
    ...
    .then(() => {
        return fetch(`https://cue-medical-management-app.herokuapp.com/posts?&userId=${this.props.activeUser}&_sort=id&_order=desc&_expand=user`)
    })
    .then(r => r.json())
    .then(posts => {
        this.setState({
    ...
    displayAllMedications = function () {
        fetch(`https://cue-medical-management-app.herokuapp.com/medications?&userId=${this.props.activeUser}&_sort=id&_order=desc&_expand=user`)
            .then(r => r.json())
            .then(medication => this.setState({medications: medication}))
    }.bind(this)

    displayAllDoctors = function () {
        fetch(`https://cue-medical-management-app.herokuapp.com/doctors?&userId=${this.props.activeUser}&_sort=id&_order=desc&_expand=user`)
            .then(r => r.json())
            .then(doctor => this.setState({doctors: doctor}))
    }.bind(this)

    componentDidMount() {
        fetch(`https://cue-medical-management-app.herokuapp.com/posts?&userId=${this.props.activeUser}&_sort=id&_order=desc&_expand=user`)
            .then(r => r.json())
