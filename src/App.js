...
class App extends Component {
    ...
    resetMeds = () => {
        ...
        this.state.medications.forEach(med => {
            console.log(med)
            falseMeds.push(fetch(`https://cue-medical-management-app.herokuapp.com/medications/${med.id}`, {
                    method: "PATCH",
                    headers: {
    ...
    displayAllPosts = function () {
        fetch(`https://cue-medical-management-app.herokuapp.com/posts?&userId=${this.state.activeUser}&_sort=id&_order=desc&_expand=user`)
            .then(r => r.json())
            .then(post => this.setState({posts: post}))
    }.bind(this)

    displayAllMedications = function () {
        fetch(`https://cue-medical-management-app.herokuapp.com/medications?&userId=${this.state.activeUser}&_sort=id&_order=desc&_expand=user`)
            .then(r => r.json())
            .then(medication => this.setState({medications: medication}))
    }.bind(this)

    displayAllDoctors = function () {
        fetch(`https://cue-medical-management-app.herokuapp.com/doctors?&userId=${this.state.activeUser}&_sort=id&_order=desc&_expand=user`)
            .then(r => r.json())
            .then(doctor => this.setState({doctors: doctor}))
    }.bind(this)

    displayAllAppointments = function () {
        fetch(`https://cue-medical-management-app.herokuapp.com/appointments?&userId=${this.state.activeUser}&_sort=id&_order=desc&_expand=user`)
            .then(r => r.json())
